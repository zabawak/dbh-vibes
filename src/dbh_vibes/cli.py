...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--autoclip",
        action="store_true",
        help="Auto-clip mode: find live-play segments (skip dead time) and write a manifest.",
    )
    parser.add_argument(
        "--clip-stride",
        type=int,
...
def _run_autoclip(args) -> int:
    ...
    for s in result.segments:
        print(f"  [{s.index:02d}] {s.start_sec:7.1f}s - {s.end_sec:7.1f}s "
              f"({s.duration_sec:.1f}s)  frames {s.start_frame}-{s.end_frame}")
    if result.clip_paths:
        print(f"Cut {len(result.clip_paths)} clip(s) into {args.out}/")
    print(f"Segment manifest: {args.out}/segments.json  (+ segments.csv)")
    return 0
